...
class Op900Detail extends AbstractStructBase
{
     ...
     */
    public function __construct(\NicholasCreativeMedia\FedExPHP\Rate\StructType\ShippingDocumentFormat $format = null, $reference = null, array $customerImageUsages = array(), $signatureName = null)
    {
        $this
            ->setFormat($format)
     ...
     * Set Format value
     * @param \NicholasCreativeMedia\FedExPHP\Rate\StructType\ShippingDocumentFormat $format
     * @return \NicholasCreativeMedia\FedExPHP\Rate\StructType\Op900Detail
     */
    ...
    public function setReference($reference = null)
    {
        ...
        if (!\NicholasCreativeMedia\FedExPHP\Rate\EnumType\CustomerReferenceType::valueIsValid($reference)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $reference, implode(', ', \NicholasCreativeMedia\FedExPHP\Rate\EnumType\CustomerReferenceType::getValidValues())), __LINE__);
        }
    ...
    public function setCustomerImageUsages(array $customerImageUsages = array())
    {
        foreach ($customerImageUsages as $op900DetailCustomerImageUsagesItem) {
            ...
            if (!$op900DetailCustomerImageUsagesItem instanceof \NicholasCreativeMedia\FedExPHP\Rate\StructType\CustomerImageUsage) {
                throw new \InvalidArgumentException(sprintf('The CustomerImageUsages property can only contain items of \NicholasCreativeMedia\FedExPHP\Rate\StructType\CustomerImageUsage, "%s" given', is_object($op900DetailCustomerImageUsagesItem) ? get_class($op900DetailCustomerImageUsagesItem) : gettype($op900DetailCustomerImageUsagesItem)), __LINE__);
            }
        }
