...
class DestinationControlDetail extends AbstractStructBase
{
     ...
     * @uses \NicholasCreativeMedia\FedExPHP\Ship\EnumType\DestinationControlStatementType::valueIsValid()
     * @uses \NicholasCreativeMedia\FedExPHP\Ship\EnumType\DestinationControlStatementType::getValidValues()
     * @throws \InvalidArgumentException
     * @param string[] $statementTypes
     * @return \NicholasCreativeMedia\FedExPHP\Ship\StructType\DestinationControlDetail
     */
    public function setStatementTypes(array $statementTypes = array())
    {
        ...
        foreach ($statementTypes as $destinationControlDetailStatementTypesItem) {
            if (!\NicholasCreativeMedia\FedExPHP\Ship\EnumType\DestinationControlStatementType::valueIsValid($destinationControlDetailStatementTypesItem)) {
                $invalidValues[] = var_export($destinationControlDetailStatementTypesItem);
            }
        }
        if (!empty($invalidValues)) {
            throw new \InvalidArgumentException(sprintf('Value(s) "%s" is/are invalid, please use one of: %s', implode(', ', $invalidValues), implode(', ', \NicholasCreativeMedia\FedExPHP\Ship\EnumType\DestinationControlStatementType::getValidValues())), __LINE__);
        }
    ...
    public function addToStatementTypes($item)
    {
        ...
        if (!\NicholasCreativeMedia\FedExPHP\Ship\EnumType\DestinationControlStatementType::valueIsValid($item)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $item, implode(', ', \NicholasCreativeMedia\FedExPHP\Ship\EnumType\DestinationControlStatementType::getValidValues())), __LINE__);
        }
