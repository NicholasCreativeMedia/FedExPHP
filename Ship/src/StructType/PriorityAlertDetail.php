...
class PriorityAlertDetail extends AbstractStructBase
{
     ...
     * @param string[] $content
     * @return \NicholasCreativeMedia\FedExPHP\Ship\StructType\PriorityAlertDetail
     */
     ...
     * @uses \NicholasCreativeMedia\FedExPHP\Ship\EnumType\PriorityAlertEnhancementType::valueIsValid()
     * @uses \NicholasCreativeMedia\FedExPHP\Ship\EnumType\PriorityAlertEnhancementType::getValidValues()
     * @throws \InvalidArgumentException
     * @param string[] $enhancementTypes
     * @return \NicholasCreativeMedia\FedExPHP\Ship\StructType\PriorityAlertDetail
     */
    public function setEnhancementTypes(array $enhancementTypes = array())
    {
        ...
        foreach ($enhancementTypes as $priorityAlertDetailEnhancementTypesItem) {
            if (!\NicholasCreativeMedia\FedExPHP\Ship\EnumType\PriorityAlertEnhancementType::valueIsValid($priorityAlertDetailEnhancementTypesItem)) {
                $invalidValues[] = var_export($priorityAlertDetailEnhancementTypesItem);
            }
        }
        if (!empty($invalidValues)) {
            throw new \InvalidArgumentException(sprintf('Value(s) "%s" is/are invalid, please use one of: %s', implode(', ', $invalidValues), implode(', ', \NicholasCreativeMedia\FedExPHP\Ship\EnumType\PriorityAlertEnhancementType::getValidValues())), __LINE__);
        }
    ...
    public function addToEnhancementTypes($item)
    {
        ...
        if (!\NicholasCreativeMedia\FedExPHP\Ship\EnumType\PriorityAlertEnhancementType::valueIsValid($item)) {
            throw new \InvalidArgumentException(sprintf('Value "%s" is invalid, please use one of: %s', $item, implode(', ', \NicholasCreativeMedia\FedExPHP\Ship\EnumType\PriorityAlertEnhancementType::getValidValues())), __LINE__);
        }
